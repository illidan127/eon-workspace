...
(defun eon-workspace--collection-encode-path (root)
  "将工作区 ROOT 路径映射为集合目录下的相对路径。
例如 /Users/foo/bar/ 映射为 Users/foo/bar/.eon.yaml（去掉前导 /）。"
  (let ((clean (directory-file-name (expand-file-name root))))
    (concat (if (string-prefix-p "/" clean)
                (substring clean 1)
              clean)
            "/" eon-workspace-config-file)))
...
(defun eon-workspace-collect-config ()
  "收集并同步所有已知工作区的 .eon.yaml 配置到 `eon-workspace-collection-path'。
在集合目录下按工作区路径镜像子目录结构存放。遍历当前打开的工作区、
已知项目列表以及集合目录下已存储的配置，为每个 .eon.yaml 建立硬连接，
实现双向同步：
- 工作区有配置而集合目录无记录 → 在集合目录下创建硬连接副本
- 集合目录有记录而工作区缺配置 → 从集合目录恢复 .eon.yaml
- 两边已有硬连接（同一 inode）→ 跳过
    ...
    (let* ((active-roots (mapcar #'eon-workspace-root eon-workspace--list))
      ...
      (dolist (root all-roots)
        (let* ((ws-file (expand-file-name eon-workspace-config-file root))
               ...
               (ws-exists (file-exists-p ws-file))
           ...
           (col-exists
            (if (file-directory-p root)
                (progn
                  ...
                  (cl-incf restored))
              (message "[eon-workspace] 工作区目录不存在（跳过）: %s" root)))
           ;; 两边都没有 —— 什么也不做
           (t nil))))
