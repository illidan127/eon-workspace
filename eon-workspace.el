...
(defconst eon-workspace-ignore-patterns-key "ignore-patterns"
  ".eon.yaml 中表示忽略模式列表的顶层 key 名。")

(defconst eon-workspace-action-key "action"
  ".eon.yaml 中表示 action 子树的顶层 key 名。")

(defconst eon-workspace-collection-files-key "collection-files"
  ".eon.yaml 中表示需纳入配置收集的文件列表的顶层 key 名。")

(defcustom eon-workspace-tag-presets
  '("frontend" "backend" "devops" "tools" "data")
  "可选标签预设列表，供 `eon-workspace-tag' 从中多选。"
  :type '(repeat string)
  :group 'eon-workspace)
...
(defcustom eon-workspace-collection-path nil
  "存储 .eon.yaml 配置集合的目录路径。
所有已知工作区的 .eon.yaml 通过硬连接汇集到此目录下，按工作区路径
镜像子目录结构存放。例如 /Users/foo/bar/ 工作区的配置存储为
<collection>/Users/foo/bar/.eon.yaml。任意一边修改即可同步到另一边。
为 nil 时禁用配置收集功能。"
  :type '(choice (const :tag "禁用" nil) directory)
  :group 'eon-workspace)

(defun eon-workspace--collection-encode-path (root &optional file)
  "将工作区 ROOT 路径（及可选的相对 FILE）映射为集合目录下的相对路径。
例如 /Users/foo/bar/ 映射为 Users/foo/bar/.eon.yaml；
/Users/foo/bar/ 加 \"src/a.json\" 映射为 Users/foo/bar/src/a.json。"
  (let ((clean (directory-file-name (expand-file-name root)))
        (rel (or file eon-workspace-config-file)))
    (concat (if (string-prefix-p "/" clean)
                (substring clean 1)
              clean)
            "/" rel)))

(defun eon-workspace--collection-decode-path (file-path col-dir)
  "从集合目录下的 FILE-PATH 解码出工作区 ROOT 路径。
给定 <col-dir>/Users/foo/bar/.eon.yaml，返回 /Users/foo/bar/。"
  (let* ((col-prefix (file-name-as-directory (expand-file-name col-dir)))
         (rel (string-remove-prefix col-prefix file-path))
         (stem (string-remove-suffix (concat "/" eon-workspace-config-file) rel)))
    (file-name-as-directory (concat "/" stem))))
...
(cl-defstruct (eon-workspace
  ...
  tags)     ;; 标签列表（字符串），持久化在 projects 文件中
...
(defvar eon-workspace--projects nil
  "已知 workspace 项目 alist（运行时缓存），格式 ((ROOT . TAG...) ...)。")
...
(defun eon-workspace--project-dirs ()
  "返回 projects alist 中所有目录字符串列表。"
  (mapcar #'car eon-workspace--projects))

(defun eon-workspace--project-entry (dir)
  "返回 DIR 在 projects alist 中的完整条目。"
  (let ((d (eon-workspace--normalize-dir dir)))
    (assoc d eon-workspace--projects)))

(defun eon-workspace--tags-get (root)
  "返回 ROOT 对应的标签列表，直接从 projects alist 读取。"
...
(defun eon-workspace--dedupe-project-alist (alist)
  "对 alist 按 car 规范化去重，保留首次出现顺序。"
  (let (seen result)
    (dolist (entry alist)
      (when (and entry (consp entry) (stringp (car entry)))
        (let ((nd (eon-workspace--normalize-dir (car entry))))
          (unless (member nd seen)
            (push nd seen)
            (push (cons nd (cdr entry)) result)))))
    (nreverse result)))

(defun eon-workspace--load-projects ()
  "从 `eon-workspace--projects-file-path' 读取项目 alist（去重）。
自动检测旧格式（扁平日录列表）并迁移为新 alist 格式。"
  (let* ((file (eon-workspace--projects-file-path))
         (raw (eon-workspace--read-list-file file)))
    (when raw
      (if (stringp (car raw))
          ;; 旧格式：扁平日录列表 → 迁移为 alist
          (setq raw (mapcar #'list (eon-workspace--dedupe-dirs raw))))
      (setq eon-workspace--projects
            (eon-workspace--dedupe-project-alist raw)))
...
(defun eon-workspace--save-projects ()
  "把项目 alist 写回 `eon-workspace--projects-file-path'。"
  ...
  (let* ((file (eon-workspace--recent-file-path))
    ...
    (when (and (not (file-readable-p file)) eon-workspace--projects)
      (setq eon-workspace--recent
            (copy-sequence (eon-workspace--project-dirs)))
...
(defun eon-workspace--remember-project (dir)
  ...
  (eon-workspace--ensure-projects-loaded)
  (let ((d (eon-workspace--normalize-dir dir)))
    (unless (assoc d eon-workspace--projects)
      (setq eon-workspace--projects
            (append eon-workspace--projects (list (list d))))
      (eon-workspace--save-projects))
...
(defun eon-workspace--known-projects ()
  "返回用于 F8 选择的项目列表：已打开的工作区优先（按最近使用排序），
其次为未打开的项目（按最近使用排序）。
不在 projects 中但出现在 recent 的项会被忽略。"
  (eon-workspace--ensure-recent-loaded)
  (eon-workspace--cleanup-dead)
  (let* ((project-dirs (eon-workspace--project-dirs))
         (all (eon-workspace--dedupe-dirs
              (append (seq-filter (lambda (d) (assoc d eon-workspace--projects))
                                  eon-workspace--recent)
                      (seq-remove (lambda (d) (member d eon-workspace--recent))
                                  project-dirs))))
         (open (seq-filter #'eon-workspace--project-open-p all))
         (closed (seq-remove #'eon-workspace--project-open-p all)))
    (append open closed)))
...
优先从已知项目列表中选择：已打开的工作区按 MRU 排最前，未打开项随后。为空则回退到 `read-directory-name'。
...
(defun eon-workspace--collection-files (root)
  "读取 ROOT 下 .eon.yaml 中 collection-files 列表。
返回相对于工作区根目录的文件路径列表。"
  (let ((file (expand-file-name eon-workspace-config-file root)))
...
若 ROOT 已绑定 workspace 则直接切换；否则创建新 workspace（独立 frame）。"
  ...
  (let* ((dir (eon-workspace--normalize-dir root))
    ...
    (if existing
        ...
        (progn
          (setf (eon-workspace-tags existing) (eon-workspace--tags-get dir))
      ...
      (let* ((frame-title (format "Workspace: %s" ws-name))
             ...
             (ws (eon-workspace--make :name ws-name :root dir :frame frame
                                      :tags (eon-workspace--tags-get dir))))
        (when reuse-current
  ...
  (interactive
   (list (progn
           ...
           (unless eon-workspace--projects
             (user-error "已知项目列表为空"))
           (completing-read "移除项目: "
                            (eon-workspace--project-dirs) nil t))))
  (eon-workspace--ensure-projects-loaded)
  (let ((d (eon-workspace--normalize-dir dir)))
    (setq eon-workspace--projects
          (assoc-delete-all d eon-workspace--projects)
          eon-workspace--recent
          (delete d eon-workspace--recent))
    (eon-workspace--save-projects)
    (eon-workspace--save-recent)
    (message "已移除项目: %s" d)))
...
;;;###autoload
(defun eon-workspace-get-tags (&optional root)
  "获取工作区的标签列表。ROOT 为工作目录绝对路径，省略时取当前工作区。
在 `interactive' 调用中以适合显示的方式输出。"
  (interactive)
  (let ((ws (or (and root (eon-workspace--find-by-root root))
                (eon-workspace-current))))
    (unless ws (user-error "未找到 workspace"))
    (let ((tags (or (eon-workspace--tags-get (eon-workspace-root ws)))))
      (if (called-interactively-p 'any)
          (if tags
              (message "标签: %s" (string-join tags ", "))
            (message "%s 无标签" (eon-workspace-name ws)))
        tags))))

;;;###autoload
(defun eon-workspace-tag ()
  "为当前工作区设置标签。从 `eon-workspace-tag-presets' 中多选，
通过 ivy 展示选项，RET 切换标签，C-g 完成。"
  (interactive)
  (require 'ivy)
  (let ((ws (eon-workspace-current)))
    (unless ws (user-error "当前 frame 未关联 workspace"))
    (let* ((root (eon-workspace-root ws))
      ...
      (cl-labels ((pick ()
        ...
        (setf (eon-workspace-tags ws) selected)
        (if selected
            (message "已设置标签: %s" (string-join selected ", "))
          (message "已清除标签"))
        (run-hooks 'eon-workspace-switch-hook)))))
...
;;;###autoload
(defun eon-workspace-collect-config ()
  "收集并同步所有已知工作区的配置文件到 `eon-workspace-collection-path'。
在集合目录下按工作区路径镜像子目录结构存放。遍历当前打开的工作区、
已知项目列表以及集合目录下已存储的配置，为 .eon.yaml 及其中
collection-files 列表所列文件各建立硬连接，实现双向同步：
- 工作区有文件而集合目录无记录 → 在集合目录下创建硬连接副本
- 集合目录有记录而工作区缺文件 → 从集合目录恢复
- 两边已有硬连接（同一 inode）→ 跳过
- 两边同名文件但不同 inode → 报告冲突并跳过"
  (interactive)
  (unless eon-workspace-collection-path
    (user-error "`eon-workspace-collection-path' 未设置"))
  (let ((col-dir (expand-file-name eon-workspace-collection-path)))
    (unless (file-directory-p col-dir)
      (make-directory col-dir t))
    (eon-workspace--ensure-projects-loaded)
    (eon-workspace--ensure-recent-loaded)
    (eon-workspace--cleanup-dead)
    (let* ((active-roots (mapcar #'eon-workspace-root eon-workspace--list))
           (stored-files (when (file-directory-p col-dir)
                           (directory-files-recursively
                            col-dir (concat (regexp-quote eon-workspace-config-file) "\\'")
                            t)))
           (stored-roots (mapcar (lambda (f)
                                   (eon-workspace--collection-decode-path f col-dir))
                                 stored-files))
           (all-roots (eon-workspace--dedupe-dirs
                       (append active-roots
                               (eon-workspace--project-dirs)
                               stored-roots)))
           (linked 0)
           (conflicts 0)
           (restored 0))
      (cl-labels ((sync-one (ws-file col-file ws-root)
                     (let ((ws-exists (file-exists-p ws-file))
                           (col-exists (file-exists-p col-file)))
                       (cond
                        ((and ws-exists col-exists (file-equal-p ws-file col-file))
                         (cl-incf linked))
                        ((and ws-exists col-exists)
                         (cl-incf conflicts)
                         (message "[eon-workspace] 配置冲突（跳过）: %s 与 %s 内容不同"
                                  ws-file col-file))
                        (ws-exists
                         (make-directory (file-name-directory col-file) t)
                         (condition-case _err
                             (add-name-to-file ws-file col-file t)
                           (file-error
                            (copy-file ws-file col-file t t t)))
                         (cl-incf linked))
                        (col-exists
                         (if (file-directory-p ws-root)
                             (progn
                               (condition-case _err
                                   (add-name-to-file col-file ws-file t)
                                 (file-error
                                  (copy-file col-file ws-file t t t)))
                               (cl-incf restored))
                           (message "[eon-workspace] 工作区目录不存在（跳过）: %s" ws-root)))
                        (t nil)))))
        (dolist (root all-roots)
          ;; 同步 .eon.yaml
          (let* ((ws-file (expand-file-name eon-workspace-config-file root))
                 (col-relative (eon-workspace--collection-encode-path root))
                 (col-file (expand-file-name col-relative col-dir)))
            (sync-one ws-file col-file root))
          ;; 同步 collection-files 列表中的文件
          (when (file-exists-p (expand-file-name eon-workspace-config-file root))
            (dolist (rel (eon-workspace--collection-files root))
              (let* ((ws-file (expand-file-name rel root))
                    (col-relative (eon-workspace--collection-encode-path root rel))
                    (col-file (expand-file-name col-relative col-dir)))
                (sync-one ws-file col-file root))))))
...
(defun eon-workspace-list ()
  ...
  (let ((buf (get-buffer-create "*Eon-Workspaces*"))
    ...
    (with-current-buffer buf
      (let ((inhibit-read-only t))
        (erase-buffer)
        (insert (format "%-2s %-24s %-24s %s\n" "" "NAME" "TAGS" "ROOT"))
        (insert (make-string 100 ?-) "\n")
        (dolist (ws eon-workspace--list)
          (let ((tags (or (eon-workspace-tags ws))))
            (insert (format "%-2s %-24s %-24s %s\n"
                            (if (eq ws current) "*" "")
                            (eon-workspace-name ws)
