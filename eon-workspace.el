...
;;   - eon-workspace-create 从已知项目列表选择：已打开则切换，未打开则创建
  ...
  (let ((file (eon-workspace--projects-file-path)))
    ...
    (with-temp-file file
      (let ((print-length nil)
        ...
        (insert ";; eon-workspace 已知项目列表，自动生成，请勿手工编辑。\n")
        (prin1 eon-workspace--projects (current-buffer))
        (insert "\n")))))

(defun eon-workspace--remember-project (dir)
  "把 DIR 加入项目列表（已存在则忽略），并持久化。"
  (eon-workspace--ensure-projects-loaded)
  (let ((d (eon-workspace--normalize-dir dir)))
    (unless (member d eon-workspace--projects)
      (setq eon-workspace--projects (cons d eon-workspace--projects))
